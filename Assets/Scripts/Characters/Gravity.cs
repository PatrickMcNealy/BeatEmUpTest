...
public class Gravity : MonoBehaviour {
	...
	void FixedUpdate () {
        ...
        for (int i = 0; i < surfaces.Length; i++)
        {
            ...
            {
                Physics2D.IgnoreCollision(parentCollider, surfaces[i].GetComponent<Collider2D>(), true);
                if (groundTriggerCollider.IsTouching(surfaces[i].GetComponent<Collider2D>()))
                {
                ...
                {
                    //IF NOT COLLIDING WITH IT, STOP BEING GROUNDED?
                    int x = 0;
                }
            }
            else
            {
                if (Physics2D.GetIgnoreCollision(parentCollider, surfaces[i].GetComponent<Collider2D>()))
                {
                    ...
                    {
                        Physics2D.IgnoreCollision(parentCollider, surfaces[i].GetComponent<Collider2D>(), false);
                    }
                }
            }
        }
